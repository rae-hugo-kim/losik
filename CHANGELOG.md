...
- **fix(music)**: Logic 바운스 저장 시 `BOUNCE_DIR`를 명시해 AIFF가 최근 저장 폴더로 흩어지는 문제를 수정했습니다. ‘바운스’ 버튼의 활성화와 저장 패널 종료를 확인해, 실제로 시작되지 않은 바운스를 성공으로 보고하지 않도록 했습니다.
- **fix(music)**: 소프트웨어 악기의 오프라인 바운스를 명시하고, 샘플 검증에 곡 중간의 완전 무음 블록 검사를 추가했습니다. After the Rain의 누락된 바운스를 프로젝트에서 다시 렌더링해 교체했습니다.
- **fix(music)**: 프로젝트 저장 위치를 명시하고 기존 이름의 자동 대치를 차단했습니다. v2 제작에는 실제 Sampler 악기 확인, 곡별 박자·길이, 의도된 쉼표를 구분하는 오디오 검증을 적용했습니다.
- **feat(music)**: v3 피드백 편곡에 승인 구간의 MIDI 연주 보존을 적용했습니다. 기타·베이스 균형, 솔로 반주 밀도, 브라스·스네어와 Rain의 길이·진입을 수정하고 기존 버전은 유지합니다.
